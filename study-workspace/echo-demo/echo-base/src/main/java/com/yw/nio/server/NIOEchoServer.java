...
public class NIOEchoServer {
    ...
    private static class EchoClientHandler implements Runnable {
        private SocketChannel client;   // 客户端通道
        ...
        public void run() {
            ...
            try{
                while (this.flag){  // 需要不断进行交互
                    buffer.clear() ;    // 清空缓冲区
                    // 向缓冲区之中读取数据
                    int readCount = this.client.read(buffer);
                    String readMassage = new String(buffer.array(), 0, readCount).trim();
                    // 回应数据信息
                    String writeMassage = "【Echo】" + readMassage +"\n";
                    if(CmdInfo.CMD_QUIT.equalsIgnoreCase(readMassage)){
                        writeMassage = "【Echo】ByeBye ... ";
                        this.flag = false;
                    }
                    // 数据输入通过缓存的形式完成，而数据的输出同样需要进行缓存操作
                    buffer.clear(); // 为了写入新的返回数据而定义
                    buffer.put(writeMassage.getBytes());    // 发送内容
                    buffer.flip();  // 重置缓冲区
